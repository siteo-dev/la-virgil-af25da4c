...
const LightRays = ({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      cleanupRef.current = () => {
        ...
        if (renderer) { try { const ext = renderer.gl.getExtension('WEBGL_lose_context'); if (ext) ext.loseContext(); const c = renderer.gl.canvas; if (c && c.parentNode) c.parentNode.removeChild(c) } catch (e) { console.warn('Cleanup error:', e) } }
  ...
  useEffect(() => {
    ...
    const { clientWidth: wCSS, clientHeight: hCSS } = containerRef.current
    const { anchor, dir } = getAnchorAndDir(raysOrigin, wCSS * r.dpr, hCSS * r.dpr)
